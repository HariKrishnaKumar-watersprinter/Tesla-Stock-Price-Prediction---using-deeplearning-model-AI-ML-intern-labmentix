...
def pred():  
    ...
    if uploaded_file is not None:
        ...
        # 1. Create a DataFrame for all predictions aligned with the Data index
        ...
        # Calculate actual trading business days for the chart markers (skips weekends)
        future_dates = [
            Data.index[end_date_idx] + pd.tseries.offsets.BDay(1), 
            Data.index[end_date_idx] + pd.tseries.offsets.BDay(5), 
            Data.index[end_date_idx] + pd.tseries.offsets.BDay(10)
        ]
        ...
        # FIX: Explicitly set the x-axis range to ensure future dates are visible
        # Find the furthest date we need to display on the chart
        ...
        if all_chart_dates:
            ...
            max_chart_date = max(all_chart_dates) + timedelta(days=3) # Add 3 days padding for text labels
